...
// ── POST: Solicitud de retiro ──
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['accion'] ?? '') === 'retiro') {
    verificarCsrf();
    if (!$afilId) {
        $_SESSION['flash'] = ['ok'=>false,'msg'=>'Tu cuenta no está vinculada a un afiliado. Contacta a administración.'];
        header('Location: ' . url('finanzas.php') . '?tab=retiro');
        exit;
    }
    ...
    // Saldo disponible
    $sStmt = $pdo->prepare("SELECT COALESCE(SUM(CASE WHEN tipo='credito' THEN monto ELSE -monto END),0) FROM movimiento_cuenta WHERE id_afiliado=:id");
    $sStmt->execute([':id' => $afilId]);
    $saldoDisp = (float)$sStmt->fetchColumn();

    if ($monto <= 0) {
        $_SESSION['flash'] = ['ok'=>false,'msg'=>'Ingresa un monto válido mayor a cero.'];
    } elseif ($tipoRetiro === 'Parcial' && $monto > $saldoDisp * 0.30) {
        $_SESSION['flash'] = ['ok'=>false,'msg'=>'El retiro parcial no puede superar el 30% de tu saldo (Bs. '.number_format($saldoDisp*0.30,2,',','.').').'];
    } elseif ($monto > $saldoDisp) {
        $_SESSION['flash'] = ['ok'=>false,'msg'=>'El monto solicitado supera tu saldo disponible.'];
    } else {
        $pdo->prepare("
            INSERT INTO solicitud_retiro (id_afiliado, tipo_retiro, monto, motivo, estado)
            VALUES (:id, :tipo, :monto, :motivo, 'pendiente')
        ")->execute([':id'=>$afilId, ':tipo'=>$tipoRetiro, ':monto'=>$monto, ':motivo'=>$motivo]);
        registrarLog('retiro_solicitado', "Retiro $tipoRetiro de Bs. $monto");
        $_SESSION['flash'] = ['ok'=>true,'msg'=>'Solicitud de retiro enviada. Será revisada por administración.'];
    }
    header('Location: ' . url('finanzas.php') . '?tab=retiro');
    exit;
}
...
$movimientos = $pdo->prepare("
    SELECT * FROM movimiento_cuenta WHERE id_afiliado=:id ORDER BY fecha DESC LIMIT 20
");
$movimientos->execute([':id' => $afilId]);
$movimientos = $movimientos->fetchAll();

// ── Saldo total ──
$saldoStmt = $pdo->prepare("
    SELECT COALESCE(SUM(CASE WHEN tipo='credito' THEN monto ELSE -monto END),0) AS saldo
    FROM movimiento_cuenta WHERE id_afiliado=:id
");
$saldoStmt->execute([':id' => $afilId]);
$saldo = $saldoStmt->fetchColumn();
  ...
  <div id="fin-panels">
    ...
    <div id="ft-retiro" class="tab-panel <?= $tab==='retiro'?'active':'' ?>">
      ...
      <div class="sc">
        ...
        <form method="POST" action="<?= url('finanzas.php') ?>" enctype="multipart/form-data">
          ...
          <div class="fr full">
            <div class="fl"><label>Motivo</label><select name="motivo"><option>Gastos médicos</option><option>Educación</option><option>Vivienda</option><option>Otro</option></select></div>
          </div>
          <div class="fr full">
            <div class="fl">
              ...
              <div class="file-drop" onclick="this.querySelector('input').click()">
                ...
                <input type="file" name="archivo" accept=".pdf,.jpg,.png" style="display:none" />
              </div>
            </div>
          </div>
